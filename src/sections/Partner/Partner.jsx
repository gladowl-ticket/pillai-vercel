...
const Partner = () => {
  ...
  return (
    <section className="bg-red-800 py-12 px-4 md:px-0">
      <div className="flex flex-wrap">
        ...
        <div className="w-full lg:w-8/12 bg-white rounded-bl-xl rounded-tl-xl px-6 py-4">
          <Swiper
            modules={[Autoplay]}
            spaceBetween={20}
            slidesPerView={3}
          ...
          >
            {partners.map((img, id) => (
              <SwiperSlide
                key={id}
                className="border-r border-gray-400 flex items-center justify-center"
              >
                <img src={img} alt={`partner-${id}`} className="pe-3" />
              </SwiperSlide>
            ))}
          </Swiper>
        </div>
